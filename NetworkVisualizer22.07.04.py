...
def main(): 
    ...
    if not NodeGroupInfo: # Create original layout of 1 node at menu if no file is found
        ...
        for i in range(0,len(NodeGroupInfo)): # Clumsy method to strip out the 4 values of the csv sprite rect
            NodeRect = str(NodeGroupInfo[i][0])
            print(NodeRect)
            NodeRectStr = [0,0,0,0]
            fComma = 0
            ...
            NodeRect_x = NodeRect[6:int(NodeRectStr[0])]
            NodeRect_y = NodeRect[(int(NodeRectStr[0])+1):int(NodeRectStr[1])]
            NodeRect_w = NodeRect[(int(NodeRectStr[1])+1):int(NodeRectStr[2])]
            NodeRect_h = NodeRect[(int(NodeRectStr[2])+1):fPara]
            NodeFinalRect = pygame.Rect((int(NodeRect_x), int(NodeRect_y)), (int(NodeRect_w), int(NodeRect_h)))
            NodeSprite = Node(GREEN, NodeTextColor, NodeTotalSizeY, NodeTotalSizeX, NodeGroupInfo[i][1], NodeTextSize)
